...
import {EjerData} from "./EjerData"

const EjemplosEquilibrio = () => {
    return (
        <>
        {
            EjerData.map((item)=>{
                return (
                    <div className={styles.divLiEjemplos} key={item.id}>
                        <ul className={styles.ulEquilibrio} key={item.id}>
                            ...
                            <li className={styles.liEquilibrio}><span className={styles.spanLiEquilibrio}>{item.spanEj3}</span>{item.pText3}<br/><br/>{item.pText3}</li>
                            <li className={styles.liEquilibrio}><span className={styles.spanLiEquilibrio}>{item.spanEj4}</span>{item.pText4}<br/><br/>{item.pText4br}</li>
                        </ul>
                    </div>
                )
            })
...
export default EjemplosEquilibrio
